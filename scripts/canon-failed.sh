...
printf '%s\n' '{"hookSpecificOutput":{"hookEventName":"PostToolUseFailure","additionalContext":"[Judaro canon] A legal-skills tool call FAILED — the firm-approved legal canon is unavailable for this request (the server errored or is unreachable). Do NOT answer Israeli-legal questions from your own training knowledge and do NOT fabricate citations, statute or regulation numbers, court-rule references, checklists, deadlines, or templates. Tell the user plainly that the legal canon is currently unreachable (an outage or an auth/connection problem) and stop — or retry the tool. Do not work around it from memory."}}'
exit 0
